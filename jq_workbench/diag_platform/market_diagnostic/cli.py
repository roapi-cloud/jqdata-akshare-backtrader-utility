...
Provides easy access to diagnostic reports, knowledge base queries, and visualizations.

Usage:
    python -m diag_platform.market_diagnostic.cli --help
    python -m diag_platform.market_diagnostic.cli run --date 2026-04-25
    python -m diag_platform.market_diagnostic.cli query --regime trend_risk_on_growth
    python -m diag_platform.market_diagnostic.cli timeline --days 30
"""
...
def cmd_run(args) -> int:
    ...
    try:
        ...
        # Initialize engine (requires data_manager - using minimal setup)
        # For a real run, you'd provide a proper data_manager
        try:
            # Try to get data manager from existing setup
            from daily_stock_analysis.src.data_provider.base import DataFetcherManager
            data_manager = DataFetcherManager()
        except ImportError:
            print("[CLI] Warning: DataFetcherManager not found, using fallback mode")
            data_manager = None

        if data_manager is None:
            print("[CLI] Error: Cannot initialize without data_manager")
            return 1
        ...
        # Save Markdown
        md_renderer = MarketDiagnosticMarkdownRenderer()
...
def cmd_query(args) -> int:
    ...
    try:
        from diag_platform.market_diagnostic.knowledge_base_manager import KnowledgeBaseManager, ReportQuery
...
def cmd_strategies(args) -> int:
    ...
    try:
        from diag_platform.market_diagnostic.strategy_mapping import (
