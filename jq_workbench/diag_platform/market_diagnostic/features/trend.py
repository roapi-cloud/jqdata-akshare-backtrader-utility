...
class TrendFeatures:
    ...
    Attributes:
        ...
        ma120: 120-day simple moving average
        ma_alignment: "多头排列" / "空头排列" / "缠绕"
        bias_ma5: (close - ma5) / ma5
        bias_ma20: (close - ma20) / ma20
        bias_ma60: (close - ma60) / ma60
        macd_dif: MACD DIF line
        macd_dea: MACD DEA line
        macd_bar: MACD bar (2 * (DIF - DEA))
        macd_signal: "金叉" / "死叉" / "中性"
        atr_20: 20-day Average True Range
        rsrs_score: Normalized RSRS score (0-1)
        near_high_20d: True if close near 20-day high (within 2%)
        break_support: True if price below MA60
        rs_vs_300: Relative strength vs CSI300
    """
    code: str
    ma5: float
    ma10: float
...
def _sma(series: np.ndarray, period: int) -> float:
    """Simple moving average of last *period* values. Returns NaN if insufficient."""
    if len(series) < period:
        return float("nan")
    return float(np.mean(series[-period:]))


def _compute_macd(closes: np.ndarray) -> Tuple[float, float, float, str]:
    """
...
def _compute_rsrs(
    ...
    """
    Compute normalized RSRS score in [0, 1].

    RSRS slope = OLS slope of high ~ low over rolling window.
    """
    n = len(highs)
...
def compute_trend_features(
    ...
    """
    Compute trend features for a single index using numpy only.

    Parameters
    ----------
    data : IndexDailyData
        Index data with close_series
    csi300_close : float, optional
    ...
    # ATR-20 (use close as approximation for high/low if not available)
...
def compute_all_trend_features(
    ...
    csi300_data: Optional[IndexDailyData] = None,
) -> Dict[str, TrendFeatures]:
    """
    Compute trend features for all indices.

    Parameters
    ----------
    index_data : Dict[str, IndexDailyData]
        Mapping of index code to IndexDailyData
    csi300_data : IndexDailyData, optional
        CSI300 data for relative strength calculation

    Returns
    -------
    Dict[str, TrendFeatures]
        Mapping of index code to TrendFeatures
    """
    csi300_close = csi300_data.close if csi300_data else None
